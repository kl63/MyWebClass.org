...
const TIMEOUT = 30000;
async function checkImagesWithAltAttribute(pageUrl) {
...
pages.forEach((page) => {
  test(`Page "${page.path}" should have an alt attribute for every img element`, async ({}) => {
    const pageUrl = `${config.use.baseURL}${page.path}`;
    const allImagesHaveAlt = await checkImagesWithAltAttribute(pageUrl);
